...
fun test (inp: String, pre: Boolean=false): String {
    //println(inp)
    val prelude = if (CEU == 99) "build/prelude-x.atm" else "build/prelude-0.atm"
...
fun main (args: Array<String>) {
    ...
    try {
        ...
        when {
            ys.containsKey("--version") -> println("atm " + VERSION)
            (xinp == null) -> println("expected filename")
            else -> {
                val f = File(xinp)
                val inps = listOf(
                    Pair(Triple(xinp,1,1), f.reader()),
                    Pair(Triple("prelude.atm",1,1), FileX("@/prelude.atm").reader())
                )
